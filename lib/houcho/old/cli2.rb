...
module Houcho
  ...
  class SpecConduct < Thor::MyHelper
    ...
    def show
      puts Houcho::Spec.elements.sort.join("\n")
    end
    ...
    def attach(*args)
      Helper.empty_args(self, shell, __method__) if args.empty?
      Houcho::Spec.attach(args, options[:roles])
    ...
    def detach(*args)
      Helper.empty_args(self, shell, __method__) if args.empty?
      Houcho::Spec.detach(args, options[:roles])
    rescue RuntimeError => e
      puts e.message
      exit!
    end
  ...
  class HostConduct < Thor::MyHelper
    ...
    def details(*args)
      Helper.empty_args(self, shell, __method__) if args.empty?
      Houcho::Console.puts_details(Houcho::Host.details(args))
    end

    desc 'show', 'show all of hosts'
    def show
      puts (Houcho::Host.elements + Houcho::CloudForecast::Host.new.all).join("\n")
    end
    ...
    def detach(*args)
      Helper.empty_args(self, shell, __method__) if args.empty?
      Houcho::Host.detach(args, options[:roles])
    rescue RuntimeError => e
      puts e.message
      exit!
    end
  ...
  class RoleConduct < Thor::MyHelper
    ...
    def rename(role, rename)
      Houcho::Role.rename(role, rename)
    rescue RuntimeError => e
      puts e.message
      exit!
    end
  ...
  class CFConduct < Thor::MyHelper
    ...
    def details(*args)
      Helper.empty_args(self, shell, __method__) if args.empty?
      Houcho::Console.puts_details(Houcho::CloudForecast::Role.details(args))
    end

    desc 'show', "show all of cloudforecast's roles"
    def show
      puts Houcho::CloudForecast::Role.all.sort.join("\n")
    end
    ...
    def attach(*args)
      Helper.empty_args(self, shell, __method__) if args.empty?
      Houcho::CloudForecast::Role.attach(args, options[:roles])
    rescue RuntimeError => e
      puts e.message
      exit!
    end
    ...
    def detach(*args)
      Helper.empty_args(self, shell, __method__) if args.empty?
      Houcho::CloudForecast::Role.detach(args, options[:roles])
    rescue RuntimeError => e
      puts e.message
      exit!
    end
